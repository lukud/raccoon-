...
Raccoon is implemeted as a driver that executes or automatically submit to a distributed cluster one of the 12+1 stages needed to polish. Wherever possible, the stages have been parallelized via a scatter-gather approach to make error corrections of large genomes possible. Each stage automatically executes the follwing one until it hits a scattered stage (yellow in the following scheme).  After the user only needs to execute stages follwing a scattered stage.
